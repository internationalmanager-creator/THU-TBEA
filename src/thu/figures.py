...
def _save(fig, name, label, source):
    """Guarda figura + devuelve metadata."""
...
def fig_03_01_helice_aurea():
    ...
    ax1.set_title(r"Golden helix 3D ($\varphi = 1.6180$)",
                  color="#FAFAFA", pad=10, fontweight="bold")
    ax1.set_xlabel("x", color="#A0A0A0"); ax1.set_ylabel("y", color="#A0A0A0")
    ax1.set_zlabel(r"$z/\varphi$", color="#A0A0A0")
    ax1.tick_params(colors="#A0A0A0"); ax1.grid(True, alpha=0.3)
    ...
    ax2.set_title(r"Spiral projection (step $\varphi$)",
                  color="#FAFAFA", pad=10, fontweight="bold")
    ax2.set_xlabel(r"$x$ ($L_p$ units)", color="#A0A0A0")
    ax2.set_ylabel(r"$y$ ($L_p$ units)", color="#A0A0A0")
...
def fig_03_02_doble_espiral():
    ...
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7",
              labelcolor="#FAFAFA", loc="upper right")
...
def _caja_diagrama(ax, cx, cy, w, h, titulo, subtitulo, color):
    ...
    box = FancyBboxPatch((cx - w/2, cy - h/2), w, h,
                          ...
                          linewidth=2, edgecolor=color, facecolor="#1A1D24")
    ax.add_patch(box)
...
def fig_03_04_sintesis_U4():
    ...
    for cy, fc, ec, titulo, subtitulo in boxes:
        ...
        ax.text(6.0, cy + 0.22, titulo, ha="center", va="center",
                fontsize=11, color="#0E1117", fontweight="bold")
        ax.text(6.0, cy - 0.3, subtitulo, ha="center", va="center",
                fontsize=9, color="#1A1D24", style="italic")
    ...
    ax.set_title(r"Synthesis THU-TBEA 5.0: $U_4 \to CY_3 \to n=88 \to$ "
                 r"phenomenology",
                 fontsize=13, color="#FAFAFA", pad=15, fontweight="bold")
...
def fig_04_01_lambert_w():
    ...
    ax.plot(x_neg, wm1, color="#E17055", linewidth=2, linestyle="--",
            label=r"$W_{-1}(z)$ (UV)")
    ax.axvline(-1/math.e, color="#A0A0A0", linestyle=":", alpha=0.5,
               label=r"$z = -1/e$")
    ax.set_xlabel(r"$z$"); ax.set_ylabel(r"$W(z)$")
    ax.set_title("Lambert-W branches (physical vs UV)")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
...
def fig_04_02_residuo():
    ...
    for m2, color in [(0.5, "#0984E3"), (1.0, "#E17055"), (2.0, "#00B894")]:
        ...
        ax.plot(a, res, color=color, linewidth=2,
                label=rf"$m^2 = {m2}$")
    ax.axhline(0, color="#A0A0A0", linewidth=0.5)
...
def fig_08_01_Hz():
    ...
    ax.set_title(r"$H(z)$: $\Lambda$CDM vs THU-TBEA", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
...
def fig_08_04_bounce():
    ...
    ax.set_title(r"ECSK cosmological bounce "
                 ...
                 color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    return _save(fig, "fig_08-04_bounce", "D",
...
def fig_11_01_tomografia():
    ...
    ax.axhline(1.0, color="#A0A0A0", linestyle="--",
    ...
    ax.set_title(r"$\beta(z)$ tomography: central falsifiable "
                 r"prediction", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7",
              labelcolor="#FAFAFA", loc="lower right")
...
def _label_color(lab):
    return {"D": "#00B894", "P": "#6C5CE7",
            "F": "#E17055", "A": "#FDCB6E"}.get(lab[0] if lab else "P", "#636E72")
...
def fig_17_01_matriz_falsacion():
    ...
    colors = [status_color.get(p[2], "#636E72") for p in preds]
    ...
    ax.set_title(r"Falsification matrix P1-P11", color="#FAFAFA")
...
def fig_17_02_bitacora_lakatosiana():
    ...
    cs = [colors.get(l[0], "#636E72") for l in labels]
    ax.scatter(xs, ys, s=400, c=cs, edgecolor="#FAFAFA",
...
def fig_17_03_patches_por_etapa():
    ...
    for b, v in zip(bars, counts):
        ax.text(b.get_x() + b.get_width()/2, v + 0.1, str(v),
                ha="center", fontsize=12, color="#FAFAFA",
                fontweight="bold")
    ax.set_xlabel("Stage")
...
def fig_13_01_prisma_funnel():
    ...
    for b, v in zip(bars, vals[::-1]):
        ax.text(v + 5, b.get_y() + b.get_height()/2, str(v),
                va="center", fontsize=12, color="#FAFAFA",
                fontweight="bold")
    ax.set_xlabel("N records")
    ax.set_title(r"PRISMA funnel: $284 \to 38$ studies",
                 color="#FAFAFA")
    return _save(fig, "fig_13-01_prisma_funnel", "D",
...
def fig_18_01_chi2_breakdown():
    ...
    for b, v in zip(bars, vals):
        ax.text(b.get_x() + b.get_width()/2, v + 2, rf"${v:.2f}$",
                ha="center", fontsize=11, color="#FAFAFA",
                fontweight="bold")
    ax.set_ylabel(r"$\chi^2$")
    ax.set_title(r"$\chi^2$ breakdown (mocks, total = 175.41)",
                 color="#FAFAFA")
    return _save(fig, "fig_18-01_chi2_breakdown", "D",
...
def fig_03_07_cadena_cy3():
    ...
    ax.set_title(r"Chain A-14: geometric derivation of $n=88$ from $CY_3$",
                 fontsize=12, pad=18, color="#FAFAFA")
...
def fig_20_01_problemas_status():
    ...
    for i, (pid, status, lab) in enumerate(probs):
        ...
        ax.scatter(xs[i], y, s=500, c=[label_color[lab]],
                   edgecolor="#FAFAFA", linewidth=1.5, zorder=3)
        ax.text(xs[i], y, pid.replace("A-", ""), ha="center",
                va="center", fontsize=8, color="#0E1117",
                fontweight="bold")
...
def generar(nombre):
    ...
    if r:
        ...
        p_target = OUT / (nombre + ".png")
        if p_actual.resolve() != p_target.resolve():
            shutil.copy2(p_actual, p_target)
            r["path"] = str(p_target.relative_to(BASE))
            r["sha256"] = _sha_file(p_target)
    return r
...
def fig_03_06_cy3_topologia():
    ...
    ax.set_aspect("equal"); ax.axis("off")
    ax.set_title(r"$M_6 = CY_3$ (correct)", fontsize=11, color="#FAFAFA")
...
def fig_03_08_h11_22_generaciones():
    ...
    ax = axes[0]
    colors = ["#A0A0A0"]*len(h11); colors[h11.index(22)] = "#E17055"
    ax.bar(h11, h11, color=colors, edgecolor="#0E1117", linewidth=1)
    ax.axvline(22, color="#E17055", linestyle="--", linewidth=2,
               label=r"$h^{1,1} = 22$ (THU)")
    ax.set_xlabel(r"$h^{1,1}$"); ax.set_ylabel("Value")
    ax.set_title(r"Hodge numbers of $CY_3$", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    ax = axes[1]
    ngen = [abs(v-19) for v in h11]
    colors = ["#E17055" if v == 22 else "#A0A0A0" for v in h11]
    ax.bar(h11, ngen, color=colors, edgecolor="#0E1117", linewidth=1)
    ax.axhline(3, color="#00B894", linestyle="--",
               label=r"$N_{gen} = 3$ (MSSM)")
    ax.set_xlabel(r"$h^{1,1}$"); ax.set_ylabel(r"$N_{gen}$")
    ax.set_title("Fermion generations", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
...
def fig_04_04_unitariedad():
    ...
    ax.semilogy(T, err, "o-", color="#6C5CE7", linewidth=2,
                ...
                markeredgecolor="#FAFAFA", markeredgewidth=1.5)
...
def fig_05_01_espacio_fases():
    ...
    for x, y, lbl, c, s in items:
        box = FancyBboxPatch((x-0.9, y-0.4), 1.8, 0.8,
                             boxstyle="round,pad=0.05",
                             edgecolor=c, facecolor="#1A1D24", linewidth=2)
        ax.add_patch(box)
        ax.text(x, y, lbl, ha="center", va="center",
                fontsize=11, color="#FAFAFA", fontweight="bold")
        ax.text(x, y-0.7, s, ha="center", fontsize=8, color="#A0A0A0")
...
def fig_05_03_cono_causalidad():
    ...
    ax.plot(t, np.abs(t), color="#6C5CE7", linewidth=2,
            label="Light cone")
    ax.axhline(0, color="#A0A0A0", linewidth=0.5)
    ax.axvline(0, color="#A0A0A0", linewidth=0.5)
    ...
    ax.set_title("Causality cone - no CTC in projected background",
                 color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7",
              labelcolor="#FAFAFA", loc="upper left")
...
def fig_05_04_grafo_poisson():
    ...
    for lbl, (x, y) in nodos.items():
        c = plt.Circle((x, y), 0.5, edgecolor="#6C5CE7",
                       facecolor="#1A1D24", linewidth=2)
        ax.add_patch(c)
        ax.text(x, y, lbl, ha="center", va="center",
                fontsize=12, color="#FAFAFA", fontweight="bold")
...
def fig_06_03_mapa_fases():
    ...
    ax.contour(G, K, beta, levels=[0], colors="#FAFAFA", linewidths=2)
...
def fig_07_02_espectro_modos():
    ...
    ax.set_title("Mode spectrum of the helical vacuum", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
...
def fig_08_04_evolucion_stiff():
    ...
    ax.set_title(r"Stiff sector evolution ($w_K = +1$)",
                 color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    return _save(fig, "fig_08-04_evolucion_stiff", "D",
...
def fig_10_03_espectro_fermionico():
    ...
    for e, m, c in zip(states, masses, colors):
        ax.scatter([0], [m], color=c, s=200, edgecolor="#FAFAFA",
...
def fig_11_03_residuo_slowroll():
    ...
    ax.set_ylabel(r"$\beta(z) / \beta_0$")
    ax.set_title("Slow-roll residual", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    ax = axes[1]
...
def fig_13_01_forest_plot():
    ...
    for i, (n, b, s) in enumerate(studies):
        ax.errorbar(b, i, xerr=s, fmt="o", color="#6C5CE7",
                    capsize=5, markersize=8, markeredgecolor="#FAFAFA",
                    markeredgewidth=1.5)
    ...
    ax.set_title(r"Forest plot: cosmic birefringence",
                 color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    return _save(fig, "fig_13-01_forest_plot", "D",
...
def fig_13_02_BAO():
    ...
    ax.set_title("BAO validation (DESI DR2)", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    ax = axes[1]
...
def fig_13_04_tension_ACT():
    ...
    for i, (b, s, c) in enumerate(zip(betas, sigmas, colors)):
        ax.errorbar(i, b, yerr=s, fmt="o", color=c, capsize=8,
                    markersize=14, markeredgecolor="#FAFAFA",
                    markeredgewidth=2)
...
def fig_15_01_cotas_paridad():
    ...
    ax.bar(x - 0.2, pred, width=0.4, color="#00B894",
           edgecolor="#0E1117", linewidth=1.5, label="THU-TBEA (prediction)")
    ax.bar(x + 0.2, cota, width=0.4, color="#E17055",
           edgecolor="#0E1117", linewidth=1.5, label="LIGO O4 (bound)")
...
def fig_16_01_polarizacion_STAR():
    ...
    ax.bar(x - 0.2, thuv, width=0.4, color="#6C5CE7",
           edgecolor="#0E1117", linewidth=1.5, label="THU-TBEA")
    ax.bar(x + 0.2, star, width=0.4, color="#E17055",
           edgecolor="#0E1117", linewidth=1.5, label="STAR 2025")
...
def fig_18_01_contornos():
    ...
    ax.scatter([0.31], [0.0198], color="#E17055", s=200, marker="*",
               edgecolor="#FAFAFA", linewidth=2, zorder=5,
    ...
    ax.set_title("Joint confidence contours (BAO + SNe + CMB)",
                 color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    return _save(fig, "fig_18-01_contornos", "D",
...
def fig_18_02_criterios():
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    ax = axes[0]
    ax.bar(x, chi2, color="#6C5CE7", edgecolor="#0E1117", linewidth=1.5)
    ax.set_xticks(x); ax.set_xticklabels(models)
    ax.set_ylabel(r"$\chi^2$")
    ax.set_title(r"$\chi^2$ per model", color="#FAFAFA")
    ...
    ax.bar(x - 0.2, aic, width=0.4, color="#0984E3",
           edgecolor="#0E1117", linewidth=1.5, label="AIC")
    ax.bar(x + 0.2, bic, width=0.4, color="#E17055",
           edgecolor="#0E1117", linewidth=1.5, label="BIC")
    ax.set_xticks(x); ax.set_xticklabels(models)
    ax.set_ylabel("Value")
    ax.set_title("AIC / BIC comparison", color="#FAFAFA")
    ax.legend(facecolor="#1A1D24", edgecolor="#6C5CE7", labelcolor="#FAFAFA")
    plt.tight_layout()
...
def fig_B_01_variacion_palatini():
    ...
    for cx, (t, s, c) in zip(centros, pasos):
        ...
        ax.text(cx, y + 0.25, t, ha="center", va="center", fontsize=9,
                color="#FAFAFA", fontweight="bold")
        ax.text(cx, y - 0.3, s, ha="center", va="center", fontsize=7, color="#A0A0A0")
    ...
    return _save(fig, "fig_B-01_variacion_palatini", "D", "tau_c = sqrt(3) tau")
def fig_E_01_metrica_FLRW():
    """Metrica FLRW."""
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.set_xlim(-3, 3); ax.set_ylim(-3, 3); ax.axis("off")
    ax.add_patch(plt.Circle((0, 0), 2.5, edgecolor="#6C5CE7", facecolor="#1A1D24",
                             linewidth=2, alpha=0.4))
    ax.add_patch(plt.Circle((0, 0), 1.5, edgecolor="#0984E3", facecolor="#1A1D24",
    ...
    for ax, titulo, mu in [(axes[0], "Phi-tau^2", "-2 i mu_phi"),
        ...
        for angle in [0, 120, 240]:
            ...
            ax.scatter([np.cos(rad)], [np.sin(rad)], color="#6C5CE7",
                       s=150, edgecolor="#FAFAFA", linewidth=1.5, zorder=5)
        ax.text(0, -0.15, mu, ha="center", va="top", fontsize=10, color="#FAFAFA")
...
def fig_M_01_PRISMA():
    ...
    for b, v in zip(bars, vals[::-1]):
        ax.text(v + 5, b.get_y() + b.get_height()/2, str(v),
                va="center", fontsize=12, color="#FAFAFA", fontweight="bold")
...
GENERADORES = {
    ...
    "fig_03-05_sintesis_U4": fig_03_05_sintesis_U4,
    "fig_03-06_cy3_topologia": fig_03_06_cy3_topologia,
    "fig_03-07_cadena_cy3": fig_03_07_cadena_cy3,
    ...
    "fig_04-02_polos_lee_wick": fig_04_02_polos_lee_wick,
    ...
    "fig_06-02_flujo_temporal": fig_06_02_flujo_temporal,
    ...
    "fig_07-02_familia_medidas": fig_07_02_familia_medidas,
    ...
    "fig_08-04_bounce": fig_08_04_bounce,
    "fig_08-04_evolucion_stiff": fig_08_04_evolucion_stiff,
    ...
    "fig_13-01_forest_plot": fig_13_01_forest_plot,
    "fig_13-01_prisma_funnel": fig_13_01_prisma_funnel,
    ...
    "fig_18-01_chi2_breakdown": fig_18_01_chi2_breakdown,
    "fig_18-01_contornos": fig_18_01_contornos,
    ...
    "fig_B-01_variacion_palatini": fig_B_01_variacion_palatini,
